...
async function initBot() {
  ...
  try {
    ...
    const ratingRegex = /([1-5])\s*(?:⭐|نجوم|stars?)/i;
    const rateRegex = /(?:قيّم|قيم|rate)\s*<@!?(\d+)>\s*([1-5])\s*(?:⭐|نجوم|stars?)?/i;
    ...
    botClient.on('messageCreate', async msg => {
      ...
      if (content.match(rateRegex) || (content.match(mentionRegex) && content.match(ratingRegex))) {
        const rateMatch = content.match(rateRegex);
        const mentionMatch = content.match(mentionRegex);
        const starMatch = content.match(ratingRegex);
        
        let ratedDiscordId = null;
        let ratingValue = null;
        
        if (rateMatch) {
          ratedDiscordId = rateMatch[1];
          ratingValue = parseInt(rateMatch[2]);
        } else if (mentionMatch && starMatch) {
          ratedDiscordId = mentionMatch[1];
          ratingValue = parseInt(starMatch[1]);
        }
        
        if (ratedDiscordId && ratingValue) {
          ...
          if (admin.length) {
            const member = dbQuery("SELECT id, name FROM members WHERE discord_tag=? OR discord_id=?", [msg.author.tag, ratedDiscordId]);
            if (member.length) {
              dbRun("INSERT INTO ratings (member_id, rated_by_id, rated_by_discord, rated_by_name, rating, reason, date) VALUES (?,?,?,?,?,?,?)",
                [member[0].id, admin[0].id, msg.author.tag, msg.author.displayName, ratingValue, content.slice(0, 100), new Date().toISOString()]);
              logAction('تقييم عضو', msg.author.displayName, member[0].name + ' - ' + ratingValue + '⭐');
              msg.reply('✅ تم تسجيل التقييم (' + ratingValue + '⭐)');
            } else {
              msg.reply('⚠️ العضو غير مسجل في النظام');
            }
          } else {
            dbRun("INSERT INTO ratings (member_id, rated_by_id, rated_by_discord, rated_by_name, rating, reason, date) VALUES (?,?,?,?,?,?,?)",
              [0, 0, msg.author.tag, msg.author.displayName, ratingValue, content.slice(0, 100), new Date().toISOString()]);
            msg.reply('✅ تم تسجيل التقييم (' + ratingValue + '⭐) - سيظهر في لوحة الإدارة');
          }
          return;
        }
      }
